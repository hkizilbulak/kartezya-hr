...
func (h *OtherRequestHandler) UploadRequestDocument(c *gin.Context) {
	...
	// Not: Dosya yükleme metodu proje genelindeki Attachment/Dys yapısına bağlanır.
...
func (h *OtherRequestHandler) GetRequestDocuments(c *gin.Context) {
	idParam := c.Param("id")
	requestID, err := strconv.ParseUint(idParam, 10, 32)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "error": "Geçersiz talep ID"})
...
func (h *OtherRequestHandler) DeleteRequestDocument(c *gin.Context) {
